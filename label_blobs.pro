FUNCTION label_blobs, img, _EXTRA=e, dilate=dilate, integer=integer
   ...
   IF integer eq 1 THEN return, labels[1:s[0],1:s[1]] ;Crop labels back to original size
   return, byte(labels[1:s[0],1:s[1]])          ;Convert to byte and crop array back to original size
END
