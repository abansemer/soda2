FUNCTION soda2_samplearea, diam, res, armwidth, numdiodes, eawmethod, smethod, wavelength, dofconst, dof=dof, eff_wid=eff_wid
   ...
   IF n_elements(dof) eq 0 THEN dof=0   ;dof==0 will compute it here instead of using custom value
   ...
   IF total(dof) eq 0 THEN dof=2*dofconst*r2overlam * 1.0e6  ;*2 is for the +/-.  Convert from m to microns.

   prht=armwidth * 1.0e4  ;convert cm to microns
   dof=dof < prht  ;Limit dof to physical distance between arms.

   ;Effective array width
   CASE eawmethod OF
