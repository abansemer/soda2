FUNCTION soda2_processbuffer, buffer, pop, pmisc
   ...
   diodegaps=intarr(num_images)        ;Number of unshaded diodes between shaded boundaries
   ...
   FOR i=0,num_images-1 DO BEGIN
      ...
      ;Walter (2025) found that no dilation works best for removing 1D2DY coincidence in large-drop wind tunnel runs.
      ...
      ;For housekeeping, to keep stuck bits using roi_orig
      IF roilen eq 1 THEN roihist=roi_orig ELSE roihist=total(roi_orig,2)
      dhist[fp:lp] += roihist
   ...
   return,{diam:diam, xsize:xsize, ysize:ysize, xextent:xextent, areasize:areasize, oned:oned, twod:twod, $
      ...
      sizecorrection:sizecorrection, dhist:dhist, nslices:nslices, missed:missed, diodegaps:diodegaps, $
