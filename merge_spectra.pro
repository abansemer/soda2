FUNCTION merge_spectra, data1, data2, crossover=crossover, tas=tas, declutter=declutter
   ...
   FOR i=0,s1[0]-1 DO conc2d_aspr_raw[i,*,*] = data1.spec2d_aspr[i,*,*]/(sagrid1*tas[i]*data1.activetime[i])
   ...
   IF n_elements(data2) ne 0 THEN BEGIN
      ...
      FOR i=0,s2[0]-1 DO conc2d2_aspr_raw[i,*,*] = data2.spec2d_aspr[i,*,*]/(sagrid2*tas[i]*data2.activetime[i])
      ...
      conc2d_aspr_raw=[[conc2d_aspr_raw[*,0:stop1,*]], [conc2d2_aspr_raw[*,start2:*,*]]]
