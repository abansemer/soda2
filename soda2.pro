PRO soda2_event, ev
   ...
   CASE uname OF
      'loadfile': BEGIN ;===========================================================================
         ...
         id=widget_info(ev.top,find='coincidencemethod')
         widget_control,id,get_value=criteria  ;See main routine below for values to compare with CASE here
         CASE op.keeplargest OF
            0:w=where(criteria eq 'Off')
            1:w=where(criteria eq 'Largest Particle (Default)')    ;Dilation of 2
            2:w=where(criteria eq 'Largest Particle (Small Dilation)')  ;Dilation of 1
      ...
      'process':BEGIN ;===========================================================================
         ...
         ;--------Coincidence Criteria
         id=widget_info(ev.top,find='coincidencemethod')
         coincidencemethodstr=widget_info(id, /combobox_gettext)
         CASE coincidencemethodstr OF
            'Off':keeplargest=0
            'Largest Particle (Default)':keeplargest=1
         ...
         op={fn:fn, date:date[0], starttime:hms2sfm(starttime[0]), stoptime:hms2sfm(stoptime[0]), format:probe.format, $
            ...
            savfile:savfile, inttime_reject:inttime_reject, eawmethod:eawmethod, stuckbits:stuckbits, water:water,$
